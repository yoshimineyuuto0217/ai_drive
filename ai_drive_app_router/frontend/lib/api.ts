...
const request = async <T>(path: string, options?: FetchInit): Promise<T> => {
...
export const articleApi = {
  list(init?: FetchInit) {
    return request<Article[]>('/api/articles', init);
  },

  getById(id: number, init?: FetchInit) {
    return request<Article>(`/api/articles/${id}`, init);
  },
